...
/* The BLOCK_SIZE is specified in the superblock, which is created by
   mkfs_sfs (That is a separate program in the ramdiskutils
   directory).  The superblock data structure must fit within a block,
   so the minimim block size is set to 128.  The default filesystem
   block size is the natural block size of the device that the
   filesystem is on, or 128, whichever is greater.  For testing, we
   use a RAMdisk with 128 byte blocks. This small block size allows
   use to test all of the code with relatively small files.

...
void kmalloc_test()
{
  ...
  *a=1000;                 
  ...
  /* c = kmalloc(9000); /* the address retrieval problem shows up even if we force
			c to be a special slab, and still at iteration 641... */
  ...
  for(i=0;i<*a;i++)
	...
	// the addresses are being stored/retrieved correctly at this point...
      }
      a2+=sizeof(int);
  ...
  for(i=0;i<500;i++) /* only try to free the first 500 addresses due to bug in
	...
	// but at this point we get 0s back for iteration 641...
      }

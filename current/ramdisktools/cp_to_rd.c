...
void read_rd(char *filename,char **buffer,int *buflen)
{
  ...
  if(stat(filename,&file_stat))
    {
      sprintf(msg,"can't stat input file <%s>",filename);
      perror(msg);
      exit(1);
    }
...
int main(int argc, char **argv)
{
  ...
  if((filefd = open(origfilename,O_RDONLY,0))<0)
    {
...
/*   int fd = vfs_open("/",O_WRONLY|O_CREAT|O_TRUNC,0);  */
  int fd = vfs_open(filename,O_WRONLY|O_CREAT,0);
  sfs_dirent d;
  while((numbytes = read(filefd,buf,bufsize))>0)
    vfs_write(fd,buf,numbytes);
  vfs_close(fd);
  close(filefd);

  /*  unmount("/"); */

   write_rd(rdname,buffer,buflen); 
